...
def Adicionar():
    ...
    # data nascimento
    data_nascimento = utils.ler_string(6,"Introduza a sua data de nascimento (dd/mm/aaaa): ")
    # id
    id = 1
    if len(leitores)>0:
...
def Editar():
    ...
    for leitor in leitores_editar:
        ...
        if op not in leitor.keys():
            print("Error: Campo indicado não existe")
            break
...
def Apagar():
    ...
    if len(leitores) == 0:
        print("Não esistem leitores")
        return
    print("Pesquisar leitor a remover: ")
    l_leitores = Pesquisar()
    # verificar se nao encontrou nenhum
    if len(l_leitores) == 0:
        print("Não foi encontrado nenhum leitor com esse nome.")
        return
    # confirmar para cada um dos livros se deseja apagar
    for leitor in l_leitores:
        print(f"Id:{leitor["id"]} | Nome: {leitor["nome"]} | email: {leitor["email"]} | idade: {leitor["idade"]} data de nascimento: {leitor["datanascimento"]} ")
...
def Listar(lista_a_listar):
    ...
    for leitor in lista_a_listar:
        print(f"Id:{leitor["id"]} | Nome: {leitor["nome"]} | email: {leitor["email"]} | idade: {leitor["idade"]} data de nascimento: {leitor["datanascimento"]} | infrações: {leitor["infracoes"]} ")
        print("-"*80)
